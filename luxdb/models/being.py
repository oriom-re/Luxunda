...
class Being:
    """
    Being reprezentuje instancję bytującą zgodnie z genotypem Soul.
    Każdy Being ma unikalny ULID i odwołuje się do Soul przez soul_hash.
    """

    ulid: str = field(default_factory=lambda: str(ulid_lib.ulid()))
    soul_hash: str = None
    global_ulid: str = field(default=Globals.GLOBAL_ULID)
    alias: str = None
    data: Dict[str, Any] = field(default_factory=dict)
    vector_embedding: Optional[List[float]] = None
    table_type: str = 'being'
    created_at: Optional[datetime] = None
    updated_at: Optional[datetime] = None

    @classmethod
    async def create(cls, soul_hash: str, data: Dict[str, Any] = None, alias: str = None) -> 'Being':
        """
    ...
    async def get_by_ulid(cls, ulid: str) -> Optional['Being']:
        ...
        Args:
            ulid: Unikalny identyfikator Being'a
    ...
    async def get_by_alias(cls, alias: str) -> List['Being']:
        """
        Ładuje wszystkie Being'i o danym aliasie.

        Args:
            alias: Alias do wyszukania

        Returns:
            Lista Being'ów o podanym aliasie
        """
        from ..repository.soul_repository import BeingRepository
    ...
    @classmethod
    async def get_all(cls) -> List['Being']:
        """
        Ładuje wszystkie Being'i z bazy danych.

        Returns:
            Lista wszystkich Being'ów
        """
        from ..repository.soul_repository import BeingRepository

        result = await BeingRepository.get_all()
        return result.get('beings', [])

    async def get_soul(self) -> Optional['Soul']:
        """
        Pobiera Soul (genotyp) dla tego Being'a.
        ...
        return await Soul.get_by_hash(self.soul_hash)
    ...
    def validate_data(self) -> List[str]:
        """
        Waliduje dane Being'a względem jego Soul.

        Returns:
            Lista błędów walidacji
        """
        # TODO: Implementacja walidacji z Soul
        return []
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Being':
        """Tworzy Being z słownika"""
        being = cls()
        being.ulid = data.get('ulid', str(ulid_lib.ulid()))
        being.soul_hash = data.get('soul_hash')
        being.global_ulid = data.get('global_ulid', Globals.GLOBAL_ULID)
        being.alias = data.get('alias')
        being.data = data.get('data', {})
    ...
    def __repr__(self):
        return f"Being(ulid={self.ulid[:8]}..., soul_hash={self.soul_hash[:8] if self.soul_hash else 'None'}..., alias={self.alias})"
